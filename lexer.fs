...
module lexer =
  ...
  let rec private collectIntLiteral nextc input literal = 
    match nextc with
    | c when List.contains c ['0'..'9'] -> 
        collectIntLiteral (nextChar input) input (literal + string c)
    | '.' -> // Check if it's a real number
        let rec collectFractional part =
            match (nextChar input) with
            | c when List.contains c ['0'..'9'] ->
                collectFractional (part + string c)
            | c -> (literal + "." + part, c)
        collectFractional ""
    | c -> (literal, c) // Return integer literal and next character
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
        // collect digits into an integer literal, check for real literal:
        let (literal,lookahead) = collectIntLiteral (nextChar input) input (string nextc)
        lexer lookahead input (if literal.Contains(".") then ("real_literal:" + literal) :: tokens else ("int_literal:" + literal) :: tokens)
